...
def step1(args, job_dir):
    ...
    datalist_path = args.datalist_path  # 用户上传标签文件
    ...
    if split_path is None:
        random_split = True
        print('[INFO] Random Split Data Set!')

    # 读取标签文件
    if datalist_path is None:
        datalist = [i for i in os.listdir(data_dir)]
    ...
    # 读取数据集划分文件
    if random_split:
        train_idx, test_idx = train_test_split(range(len(datalist)), test_size=0.2, random_state=42)
        splits = ['train' if i in train_idx else 'test' for i in range(len(datalist))]
